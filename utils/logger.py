...
class Logger:
    def __init__(self, args):
        ...
        elif os.path.exists(os.path.join(self.dir_save, 'last_{}.pth'.format(str(args.seed)))) and os.path.exists(
                self.dir_log):
    ...
    def add_validation_logs(self, step):
        if self.args.task_type == "binary":
            ...
            os.system("echo  \'auc: {}, apr: {}, f1_score: {}\'".format(str(self.best_result_so_far[0]),
                                                                        str(self.best_result_so_far[1]),
                                                                        str(self.best_result_so_far[2])))
            os.system("echo  \'tpr: {}, fnr: {}, tnr: {}, fpr: {}\'".format(str(self.best_results[0]),
                                                                            str(self.best_results[1]),
                                                                            str(self.best_results[2]),
                                                                            str(self.best_results[3])))
            ...
            os.system("echo  \'##### Current Validation results #####\'")
            os.system("echo  \'multi_weighted: auc: {}, apr: {}, f1_score: {}\'".format(str(result[0]), str(result[2]),
                                                                                        str(result[4])))
            os.system(
                "echo  \'multi_unweighted: auc: {}, apr: {}, f1_score: {}\'".format(str(result[1]), str(result[3]),
                                                                                    str(result[5])))
            ...
            for idx, seizure in enumerate(seizure_list):
                results.append("Label:{} auc:{} apr:{} f1:{} tpr:{} fnr:{} tnr:{} fpr:{} fdr:{} ppv:{}".format(seizure,
                    str(result_aucs[idx]), str(result_aprs[idx]), str(result_f1scores[idx]), str(tprs[idx]), str(fnrs[idx]),
                    str(tnrs[idx]), str(fprs[idx]), str(fdrs[idx]), str(ppvs[idx])))
            ...
            os.system("echo  \'##### Best Validation results in history #####\'")
            os.system("echo  \'multi_weighted: auc: {}, apr: {}, f1_score: {}\'".format(str(self.best_result_so_far[0]),
                                                                                        str(self.best_result_so_far[2]),
                                                                                        str(self.best_result_so_far[4])))
            os.system(
                "echo  \'multi_unweighted: auc: {}, apr: {}, f1_score: {}\'".format(str(self.best_result_so_far[1]),
                                                                                    str(self.best_result_so_far[3]),
                                                                                    str(self.best_result_so_far[5])))
    ...
    def save(self, model, optimizer, step, epoch, last=None):
        ckpt = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'best_step': step, 'last_step': last,
                'score': self.best_auc, 'epoch': epoch}
    ...
    def test_result_only(self):
        ...
        if self.args.task_type == "binary" or self.args.task_type == "binary_noslice":
            ...
            for idx, seizure in enumerate(seizure_list):
                results.append("Label:{} auc:{} apr:{} f1:{} tpr:{} fnr:{} tnr:{} fpr:{} fdr:{} ppv:{}".format(seizure,
                        str(result_aucs[idx + 1]), str(result_aprs[idx + 1]), str(result_f1scores[idx + 1]), str(tprs[idx + 1]),
                        str(fnrs[idx + 1]), str(tnrs[idx + 1]), str(fprs[idx + 1]), str(fdrs[idx + 1]), str(ppvs[idx + 1])))
